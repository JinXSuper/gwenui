...
export default async function ComparisonTableBlockPage() {
  const { content } = await getBlockMdxContent("comparison-table");

  let files: SourceFile[] = [];
  try {
    const blockDir = path.join(process.cwd(), "../../blocks/comparison-table");
    files = [
      { name: "ComparisonTable.tsx", path: "comparison-table/ComparisonTable.tsx", content: fs.readFileSync(path.join(blockDir, "ComparisonTable.tsx"), "utf-8"), language: "tsx" },
      { name: "index.ts",            path: "comparison-table/index.ts",            content: fs.readFileSync(path.join(blockDir, "index.ts"), "utf-8"),            language: "typescript" },
      { name: "block.json",          path: "comparison-table/block.json",          content: fs.readFileSync(path.join(blockDir, "block.json"), "utf-8"),          language: "json" },
    ];
  } catch { files = []; }
  ...
  return (
    <DocsSplitLayout
      ...
      sourceFiles={files}
      rightContent={<PreviewPanel src="/preview/comparison-table" />}
      type="component"
    />
  );
}

...
export default async function BeforeAfterSliderBlockPage() {
  // Load MDX content
  const { content } = await getBlockMdxContent("before-after-slider");

  // Load source files
  let files: SourceFile[] = [];
  try {
    const blockDir = path.join(process.cwd(), "../../blocks/before-after-slider");
    files = [
      { name: "before-after-slider.tsx", path: "before-after-slider/before-after-slider.tsx", content: fs.readFileSync(path.join(blockDir, "before-after-slider.tsx"), "utf-8"), language: "tsx" },
      { name: "index.ts",                path: "before-after-slider/index.ts",                content: fs.readFileSync(path.join(blockDir, "index.ts"), "utf-8"),                language: "typescript" },
      { name: "block.json",              path: "before-after-slider/block.json",              content: fs.readFileSync(path.join(blockDir, "block.json"), "utf-8"),              language: "json" },
    ];
  } catch { files = []; }
  ...
  return (
    <DocsSplitLayout
      ...
      sourceFiles={files}
      rightContent={<PreviewPanel src="/preview/before-after-slider" />}
      type="component"
    />
  );
}

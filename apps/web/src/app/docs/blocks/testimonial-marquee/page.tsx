...
export default async function TestimonialMarqueeBlockPage() {
  const { content } = await getBlockMdxContent("testimonial-marquee");

  let files: SourceFile[] = [];
  try {
    const blockDir = path.join(process.cwd(), "../../blocks/testimonial-marquee");
    files = [
      { name: "index.tsx",             path: "testimonial-marquee/index.tsx",                          content: fs.readFileSync(path.join(blockDir, "index.tsx"), "utf-8"),                          language: "tsx" },
      { name: "TestimonialCard.tsx",   path: "testimonial-marquee/components/TestimonialCard.tsx",      content: fs.readFileSync(path.join(blockDir, "components/TestimonialCard.tsx"), "utf-8"),      language: "tsx" },
      { name: "MarqueeRow.tsx",        path: "testimonial-marquee/components/MarqueeRow.tsx",           content: fs.readFileSync(path.join(blockDir, "components/MarqueeRow.tsx"), "utf-8"),           language: "tsx" },
      { name: "block.json",            path: "testimonial-marquee/block.json",                          content: fs.readFileSync(path.join(blockDir, "block.json"), "utf-8"),                          language: "json" },
    ];
  } catch { files = []; }
  ...
  return (
    <DocsSplitLayout
      ...
      sourceFiles={files}
      rightContent={<PreviewPanel src="/preview/testimonial-marquee" />}
      type="component"
    />
  );
}

...
export default async function BentoGridBlockPage() {
  const { content } = await getBlockMdxContent("bento-grid");

  let files: SourceFile[] = [];
  try {
    const blockDir = path.join(process.cwd(), "../../blocks/bento-grid");
    files = [
      { name: "BentoGrid.tsx", path: "bento-grid/BentoGrid.tsx", content: fs.readFileSync(path.join(blockDir, "BentoGrid.tsx"), "utf-8"), language: "tsx" },
      { name: "index.ts",      path: "bento-grid/index.ts",      content: fs.readFileSync(path.join(blockDir, "index.ts"), "utf-8"),      language: "typescript" },
      { name: "block.json",    path: "bento-grid/block.json",    content: fs.readFileSync(path.join(blockDir, "block.json"), "utf-8"),    language: "json" },
    ];
  } catch { files = []; }
  ...
  return (
    <DocsSplitLayout
      ...
      sourceFiles={files}
      rightContent={<PreviewPanel src="/preview/bento-grid" />}
      type="component"
    />
  );
}

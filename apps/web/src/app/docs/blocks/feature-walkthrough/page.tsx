...
export default async function FeatureWalkthroughBlockPage() {
  const { content } = await getBlockMdxContent("feature-walkthrough");

  let files: SourceFile[] = [];
  try {
    const blockDir = path.join(process.cwd(), "../../blocks/feature-walkthrough");
    files = [
      { name: "FeatureWalkthrough.tsx", path: "feature-walkthrough/FeatureWalkthrough.tsx", content: fs.readFileSync(path.join(blockDir, "FeatureWalkthrough.tsx"), "utf-8"), language: "tsx" },
      { name: "index.ts",               path: "feature-walkthrough/index.ts",               content: fs.readFileSync(path.join(blockDir, "index.ts"), "utf-8"),               language: "typescript" },
      { name: "block.json",             path: "feature-walkthrough/block.json",             content: fs.readFileSync(path.join(blockDir, "block.json"), "utf-8"),             language: "json" },
    ];
  } catch { files = []; }
  ...
  const leftContent = (
    <div className="w-full flex flex-col flex-1 py-4">
      ...
      <DocsPrevNext />
      <DocsFooter tags={["Next.js 16", "TypeScript", "Framer Motion", "Lucide"]} />
    </div>
  );
  ...
  return (
    <DocsSplitLayout
      ...
      sourceFiles={files}
      rightContent={<PreviewPanel src="/preview/feature-walkthrough" />}
      type="component"
    />
  );
}

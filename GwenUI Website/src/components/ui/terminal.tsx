...
const tokenColors: Record<TokenType, string> = {
  ...
  flag: "text-[var(--terminal-purple)]/90",
  string: "text-[var(--terminal-pink)]/90",
  number: "text-emerald-400/90",
  operator: "text-white/60",
  path: "text-sky-400/90",
  variable: "text-pink-400/90",
...
export function Terminal({
  ...
  const PromptTop = () => (
    <div className="flex items-center text-[var(--terminal-cyan)] select-none text-[11px] opacity-70 mb-0.5">
      <span>┌──(</span>
      <span className="text-[var(--terminal-blue)] font-bold">gwen</span>
      <span className="text-white/60 mx-0.5">㉿</span>
      <span className="text-[var(--terminal-cyan)]">cli</span>
      <span>)-[</span>
  ...
  const PromptBottom = () => (
    <span className="text-[var(--terminal-cyan)] select-none mr-3">
      └─<span className="text-[var(--terminal-orange)] font-bold ml-1">$</span>
    </span>
  );
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "mx-auto w-full max-w-[850px] font-mono text-[13px] md:text-[14px]",
        className,
      )}
    >
      <div className="overflow-hidden rounded-xl border border-white/5 bg-[var(--terminal-bg)] shadow-2xl relative">
        {/* Noise overlay */}
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-white/[0.03] border-b border-white/[0.05]">
          ...
          <div className="text-[10px] text-white/30 font-mono uppercase tracking-[0.3em] font-medium">
            Terminal — {username} — 80x24
          </div>
          <div className="w-12" />
        </div>
        ...
        <div
          ref={contentRef}
          className="no-visible-scrollbar h-[400px] overflow-y-auto p-6 md:p-8 font-mono scroll-smooth relative z-10"
        >
          {lines.map((line, i) => (
            <div key={i} className="leading-6 whitespace-pre-wrap mb-4 flex flex-col items-start">
              {line.type === "command" ? (
                <>
                  <PromptTop />
                ...
                <div className="text-white/40 border-l border-white/5 pl-4 ml-2 my-1 italic">
                  {line.content}
                </div>
              )}
            </div>
          ...
          {phase === "typing" && (
            <div className="leading-6 whitespace-pre-wrap flex flex-col items-start">
              <PromptTop />
              <div className="flex items-center">
                <PromptBottom />
                <div className="relative">
                  <SyntaxHighlightedText text={currentText} />
                  <span className="inline-block w-[7px] h-[15px] bg-white/60 ml-1 translate-y-[2px]" />
                </div>
              </div>
            </div>
          )}
          ...
          {(phase === "done" ||
            ...
            <div className="leading-6 whitespace-pre-wrap flex flex-col items-start">
              ...
              <div className="flex items-center">
                ...
                <span
                  className={cn(
                    "w-[7px] h-[15px] bg-white/60 ml-1 translate-y-[2px] transition-opacity duration-100",

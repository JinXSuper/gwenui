        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex items-center h-[52px] px-4 border-b border-[var(--docs-border)]">
              <Search size={18} className="text-[var(--muted-foreground)] mr-3 flex-shrink-0" />
              <input
                type="text"
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Search docs, blocks, components..."
                className="flex-1 bg-transparent border-0 outline-none text-white text-[15px] font-sans placeholder:text-[var(--muted-foreground)] h-full"
                autoFocus
              />
              <button
                onClick={() => setIsOpen(false)}
                className="text-[10px] font-mono px-2 py-1 rounded bg-white/[5%] border border-[var(--docs-border-faint)] text-[var(--docs-text-dim)] hover:text-white transition-colors cursor-pointer select-none"
              >
                Esc
              </button>
            </div>
                          ...
                          return (
                            <div
                              ...
                              className={`h-10 px-3 flex items-center justify-between border-l-2 transition-all duration-100 cursor-pointer rounded-none ${
                                isSelected
                                  ? "border-[var(--primary)] bg-[var(--docs-elem-active)] text-white font-medium"
                                  : "border-transparent bg-transparent hover:bg-[var(--docs-elem-hover)] text-[var(--docs-text-body)] hover:text-white"
                              }`}
                            >

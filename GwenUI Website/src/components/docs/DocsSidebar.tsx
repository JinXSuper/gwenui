        ...
        {nav.map((section) => (
          <div key={section.group} className="flex flex-col gap-1.5 mt-6 first:mt-0">
            ...
            <div className="flex items-center justify-between px-0 mb-2">
              ...
              {section.badge && (
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20 shadow-[0_0_10px_var(--primary)/10]">
                  {section.badge}
                </span>
              )}
            </div>
              ...
              {section.items.map((item) => {
                ...
                return (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      className={`block text-[13px] py-1.5 transition-all duration-150 rounded-none bg-transparent hover:bg-transparent ${active
